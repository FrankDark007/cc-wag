...
### Buttons/Lists DO NOT WORK in Baileys
WhatsApp blocked `buttonsMessage` and `listMessage` for non-WABA accounts. All interactive UIs must use:
- Numbered text menus (already doing this for /model)
- Polls (fully supported, see below)
- Text-based confirmation ("Reply Y/N")

### PLAN-F21: Polls for Scheduling & Decisions

**Goal**: Use WhatsApp polls for crew scheduling, client appointment preference, and team decisions.

#### Files to Modify
1. `src/adapters/whatsapp.js` — Add `sendPoll()` method
2. `src/tools/gateway-mcp.js` — Add `send_poll` MCP tool
3. `src/agent/claude-agent.js` — Add tool to allowed list

**Implementation**:

```js
// In whatsapp.js:
async sendPoll(chatId, name, options, selectableCount = 1) {
  if (!this.sock) throw new Error('WhatsApp not connected')
...
```js
// MCP tool in gateway-mcp.js:
tool(
  'send_poll',
  'Send a WhatsApp poll for scheduling, voting, or collecting preferences.',
  {
    chat_id: z.string().describe('WhatsApp chat ID'),
    question: z.string().describe('Poll question'),
    options: z.array(z.string()).describe('Poll options (2-12 choices)'),
    multi_select: z.boolean().optional().describe('Allow multiple selections (default: single)')
  },
  async ({ chat_id, question, options, multi_select }) => {
    // ... send via adapter.sendPoll()
  }
)
```

**Use cases**:
- "When works best for the inspection?" → Morning/Afternoon/Evening poll
- "Which job site first today?" → poll to crew group
- "Approve this scope?" → Yes/No/Need Changes poll
- "Team lunch?" → restaurant options poll

**Effort**: ~2 hours

### PLAN-F22: Group Management for Job Sites

**Goal**: Auto-create WhatsApp groups for job sites with crew + customer.
...
MCP tool: `create_job_group(client_name, address, crew_phones[])`

**Use case**: "Create a group for the Smith job" → creates "FD: Smith - 123 Main St" group, adds crew + customer, pins address and schedule.

**Effort**: ~4 hours

### PLAN-F23: Read Receipts

**Goal**: Mark messages as read after Atlas processes them.

```js
// In handleMessage(), after emitting message:
if (this.sock) {
  await this.sock.readMessages([msg.key])
}
```

**Effort**: ~15 minutes. Trivial but professional.

### WhatsApp AI Policy Note

Meta's 2026 policy prohibits general-purpose AI chatbots on WABA. Atlas runs via Baileys (unofficial), so not subject to WABA TOS. If migrating to official API in the future, frame Atlas as a structured business tool (appointment scheduling, job updates, crew coordination, document delivery) — not a general chatbot.

---

*End of Implementation Plans v1.1 — Updated with research agent findings*
